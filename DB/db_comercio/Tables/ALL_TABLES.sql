...
CREATE TABLE IF NOT EXISTS `DB_COMERCIO`.`PESSOA` (
  ...
  `TPO_PESSOA` ENUM('F', 'J') NULL DEFAULT NULL COMMENT 'F - Física; J - Jurídica',
...
CREATE TABLE IF NOT EXISTS `DB_COMERCIO`.`CLIENTE` (
  ...
  `TPO_SEXO` ENUM('M', 'F', 'N') NULL DEFAULT NULL COMMENT 'M - Masculino; F - Feminino; N - Neutro',
...
CREATE TABLE IF NOT EXISTS `DB_COMERCIO`.`OPERACAO` (
  ...
  `TPO_OPERACAO` ENUM('C', 'V') NULL COMMENT 'C - Compra; V - Venda',
...
CREATE TABLE IF NOT EXISTS `DB_COMERCIO`.`USUARIO` (
  ...
  `TPO_USUARIO` ENUM('A', 'F') NULL DEFAULT NULL comment 'A - Admin; F - funcionário',
