...
CREATE TABLE IF NOT EXISTS `DB_COMERCIO`.`OPERACAO` (
  ...
  `TPO_STATUS` ENUM('P', 'O') NULL  COMMENT 'P - pedido (compra ou venda); O - orçamento\n',
  `COD_PESSOA` INT NULL ,
  `FLG_PAGO` TINYINT(4) NULL ,
...
CREATE TABLE IF NOT EXISTS `DB_COMERCIO`.`USUARIO` (
  ...
  `NME_USUARIO` VARCHAR(100) NULL ,
  `DSC_LOGIN` VARCHAR(30) NOT NULL,
  `DSC_SENHA` VARCHAR(255) NULL ,
  `TPO_USUARIO` ENUM('A', 'F') NULL  comment 'A - Admin; F - funcionário',
  `FLG_ATIVO` TINYINT NULL ,
  `COD_PESSOA` INT NULL,
